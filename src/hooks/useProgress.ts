...
export const useProgress = (offset?) => {
    ...
    return {
        ...
        actionInProgress: async(action, paramId) => {
            const res = await repository.post('progress/action-inprogress',{
                data: {
                    action: action,
                    id: paramId,
                }
            ...
            if(res.status === 200) {
                // 先頭ページへ戻る
                // const res = await repository.get(`progress/fetch-in-progress/${initialPage}`).catch(error => error.responnse)
                // if(res.status !== 200) {
                //     setHttpStatus(res.status);
                // }
        ...
        actionInEscalation: async(action, paramId) => {
            const res = await repository.post('progress/action-inescalation',{
                data: {
                    action: action,
                    id: paramId
                }
        ...
        downloadFile: async(data) => {
            const res = await repository.post('progress/fetch-file',{data:data},{responseType: "blob"}).catch(error => error.response)
