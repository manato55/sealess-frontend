...
export const AuthCheck = ({children}) => {
    ...
    useEffect(() => {
        if(user) {
            if((!router.pathname.match('/login') && !user?.id) && (!router.pathname.match('/register/*') && !user?.id) && (!router.pathname.match('/password-issuance/*') && !user?.id)) {
                router.push( '/login')      
                return 
            ...
            if((router.pathname === '/admin' && user?.user_type !== 0) || (router.pathname === '/dep-admin' && user.user_type !== 1)) {
                router.push( '/') 
                return
            }
